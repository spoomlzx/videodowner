...
import com.android.iplayer.controller.GestureController.SimpleOnGesture
...
abstract class GestureController(context: Context?) : BaseController(context), OnTouchListener {
    ...
    private inner class SimpleOnGesture : SimpleOnGestureListener() {
        override fun onDown(e: MotionEvent): Boolean {
            val edge = PlayerUtils.getInstance().isEdge(parentContext, e)
            // ILogger.d(TAG,"onDown-->isPlayering:"+isPlayering()+",edge:"+edge+",mIsGestureEnabled:"+mIsGestureEnabled+",e:"+e.getAction());
            //不处于播放状态 || 关闭了手势 || 处于屏幕边沿
            if (!isPlayering || !mIsGestureEnabled || edge) return true
...
//        ILogger.d(TAG,"onScroll-->IsGestureEnabled:"+mIsGestureEnabled+",mCanSlide:"+mCanSlide+",mFirstTouch:"+mFirstTouch);
            //不处于播放状态 || 关闭了手势 || 关闭了滑动手势 || 锁住了屏幕 || 处于屏幕边沿
            if (!isPlayering || !mIsGestureEnabled || !mCanSlide || isLocked || PlayerUtils.getInstance().isEdge(parentContext, e1)) {
                return true
            }
            ...
            if (mFirstTouch) {
                ...
                if (!mChangePosition) {
                    //半屏宽度
                    val halfScreen = PlayerUtils.getInstance().getScreenWidth(context) / 2
                    if (e2.x > halfScreen) {
                        mChangeVolume = true
                    } else {
    ...
    override fun onTouch(view: View, event: MotionEvent): Boolean {
        return if (null != mGestureDetector) {
    ...
    private fun slideToChangePosition(deltaX: Float) {
        ...
        if (null != mVideoPlayerControl) {
            ...
            val duration = mVideoPlayerControl.duration.toInt()
            val currentPosition = mVideoPlayerControl.currentPosition.toInt()
