...
import org.nudt.videoplayer.controls.ControlStatusView.OnStatusListener
...
class ControWindowView(context: Context?) : BaseControlWidget(context), View.OnClickListener {
    ...
    override fun showControl(isAnimation: Boolean) {
        if (isVisible && null != mController) {
            if (mController!!.visibility != VISIBLE) {
                if (isAnimation) {
                    AnimationUtils.getInstance().startAlphaAnimatioFrom(mController, animationDuration, false, null)
                } else {
    ...
    override fun hideControl(isAnimation: Boolean) {
        if (isVisible && null != mController) {
            if (mController!!.visibility != GONE) {
                if (isAnimation) {
                    AnimationUtils.getInstance().startAlphaAnimatioTo(mController, animationDuration, false) { mController!!.visibility = GONE }
                } else {
    ...
    override fun onPlayerScene(playerScene: Int) {
        ...
        if (isWindowScene) {
            show()
            findViewById<View>(R.id.window_fullscreen).visibility = if (isWindowGlobalScene(playerScene)) VISIBLE else GONE
        } else {
            hide()
        }
    }
