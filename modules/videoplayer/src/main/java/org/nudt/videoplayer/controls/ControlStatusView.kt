...
class ControlStatusView : BaseControlWidget {
    ...
    override fun initViews() {
        ...
        PlayerUtils.getInstance().setOutlineProvider(findViewById(R.id.player_status_btn), PlayerUtils.getInstance().dpToPxInt(18f).toFloat())
        btnContinue.setOnClickListener(OnClickListener { //点击事件优先回调给监听器处理
            ...
            if (null != mControlWrapper) {
                when (mScene) {
                    SCENE_MOBILE -> {
                        IVideoManager.getInstance().isMobileNetwork = true
    ...
    fun setScene(scene: Int, tipsStr: String?, btnStr: String?) {
        ...
        tips.text = PlayerUtils.getInstance().formatHtml(getTipsStr(scene, tipsStr))
        btn.text = PlayerUtils.getInstance().formatHtml(getBtnStr(scene, btnStr))
    }
    ...
    fun setSceneType(sceneType: Int) {
        val textSize14 = PlayerUtils.getInstance().dpToPxInt(14f)
        val textSize13 = PlayerUtils.getInstance().dpToPxInt(13f)
        val textSize16 = PlayerUtils.getInstance().dpToPxInt(16f)
        val tips = findViewById<View>(R.id.player_status_tips) as TextView
        tips.setTextSize(TypedValue.COMPLEX_UNIT_PX, if (1 == sceneType) textSize14.toFloat() else textSize16.toFloat())
        val paddingLeft12 = PlayerUtils.getInstance().dpToPxInt(12f)
        val paddingLeft22 = PlayerUtils.getInstance().dpToPxInt(22f)
        val btn = findViewById<View>(R.id.player_status_btn) as TextView
        btn.setTextSize(TypedValue.COMPLEX_UNIT_PX, if (1 == sceneType) textSize13.toFloat() else textSize16.toFloat())
        ...
        layoutParams.height = PlayerUtils.getInstance().dpToPxInt(if (1 == sceneType) 26f else 36f)
        layoutParams.setMargins(0, PlayerUtils.getInstance().dpToPxInt(if (1 == sceneType) 15f else 20f), 0, 0)
        btn.layoutParams = layoutParams
        PlayerUtils.getInstance().setOutlineProvider(btn, PlayerUtils.getInstance().dpToPxInt(if (1 == sceneType) 13f else 18f).toFloat())
    }
...
//        }
    ...
    companion object {
        const val SCENE_MOBILE = 1 //移动网络播放提示
        const val SCENE_COMPLETION = 2 //试看结束
        const val SCENE_ERROR = 3 //播放失败
    }
}