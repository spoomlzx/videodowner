...
class ControlListView(context: Context?) : BaseControlWidget(context), View.OnClickListener {
    private var mSurplusDuration //剩余时间
            : TextView? = null
    ...
    override fun onProgress(currentDurtion: Long, totalDurtion: Long) {
        try {
            if (null != mSurplusDuration) mSurplusDuration!!.text = PlayerUtils.getInstance().stringForAudioTime(totalDurtion - currentDurtion)
        } catch (e: Throwable) {
            e.printStackTrace()
        }
    }
    ...
    fun showSoundMute(showSound: Boolean, soundMute: Boolean) {
        ...
        if (null != mControlWrapper) mControlWrapper.isSoundMute = soundMute //UI状态将在onMute回调中处理
    }

    override fun onReset() {
        if (null != mSurplusDuration) mSurplusDuration!!.text = PlayerUtils.getInstance().stringForAudioTime(0)
    }
}